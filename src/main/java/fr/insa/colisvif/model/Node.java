...
/**
 * A class representing a Node, formed with an id, a longitude and latitude in
 * degrees, and a {@link List} of successors ({@link Section}).
 */
public class Node {
     ...
     * @throws IllegalArgumentException if the latitude is not between -90 and
     * 90 and/or the longitude is not between -180 and 180.
     */
    public Node(long id, double latitude, double longitude)
        ...
        if (this.latitude < -90 || this.latitude > 90) {
            throw new IllegalArgumentException("The latitude must be between "
                    + "-90 and 90, got " + latitude);
        }
        if (this.longitude > 180 || this.longitude < -180) {
            throw new IllegalArgumentException("The longitude must be between "
                    + "-180 and 180, got " + longitude);
        }
    }
    ...
    /**
     * Add a {@link Section} to the {@link List} of successors of the
     * current {@link Node}.
     * @param section the {@link Section} to add.
     * @throws IdException if the origin of the new {@link Section} is
     * different from the id of the current {@link Node}.
     */
    public void addToSuccessors(Section section) throws IdException {
        if (section.getOrigin() == this.id) {
            this.successors.add(section);
        } else {
            throw new IdException("The origin of the new section does not "
                    + "correspond to the origin of this Node. Got "
                    + section.getOrigin() + " instead of " + this.id);
        }
    }
     ...
     * @throws IllegalArgumentException if the longitude is less or more than
     * 180
     */
    ...
    public String toString() {
        String result = "ID : " + id + " | Latitude : " + latitude
                + " | Longitude : " + longitude + "\n";
        return result;
    }
